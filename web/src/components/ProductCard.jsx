...
function Stars({ rating }) {
  ...
  return (
    <div className="pcard__stars" aria-label={`${rating} out of 5 stars`}>
      {Array.from({ length: full  }, (_, i) => <span key={`f${i}`} className="star star--full">★</span>)}
      {half && <span className="star star--half">★</span>}
      {Array.from({ length: empty }, (_, i) => <span key={`e${i}`} className="star star--empty">★</span>)}
...
export default function ProductCard({ product }) {
  ...
  const add = (e) => { e.preventDefault(); setQty(1); };
  const inc = (e) => { e.preventDefault(); setQty(q => q + 1); };
  const dec = (e) => { e.preventDefault(); setQty(q => Math.max(0, q - 1)); };
    ...
    <article className="pcard">
      ...
      <div className="pcard__body">
        ...
        <div className="pcard__footer">
          ...
          {qty === 0 ? (
            <button className="pcard__btn" onClick={add}>Add to Cart</button>
          ) : (
